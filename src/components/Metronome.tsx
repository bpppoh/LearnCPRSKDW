...
export const Metronome: React.FC = () => {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-brand-orange-200 bg-gradient-to-b from-white via-orange-50/30 to-brand-orange-50/50 p-6 sm:p-8 shadow-lg">
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 rounded-full border border-brand-orange-300/80 bg-brand-orange-100/80 px-3.5 py-1 text-xs font-semibold text-brand-orange-800">
        ...
        <div className="mt-6 flex flex-col items-center">
          <div
            className={`relative flex h-32 w-32 items-center justify-center rounded-full transition-all duration-100 ${
              isFlashing
          ...
          >
            <Heart
              className={`h-16 w-16 transition-transform duration-75 ${
                isFlashing ? 'scale-125 fill-white text-white' : 'text-brand-orange-500 fill-brand-orange-100'
              }`}
            />
            {isPlaying && (
              <span className="absolute bottom-2 text-xs font-bold uppercase tracking-wider">
                Beat {currentBeat}/30
              </span>
            )}
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <span className="rounded-lg bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700 border border-slate-200">
              รอบที่: <strong className="text-brand-orange-600">{cycleCount}</strong>
            </span>
            <span className="rounded-lg bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700 border border-slate-200">
              จังหวะปัจจุบัน: <strong className="text-brand-blue-600">{currentBeat || '-'}</strong> / 30 ครั้ง
            </span>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          <button
            onClick={togglePlay}
            className={`flex items-center gap-2 rounded-2xl px-6 py-3.5 text-base font-bold shadow-lg transition-all active:scale-95 ${
              isPlaying
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 active:scale-95"
          ...
          >
            {isMuted ? (
              ...
              <>
                <Volume2 className="h-5 w-5 text-brand-blue-600" />
                <span>เสียงบี๊บ</span>
              </>
            )}
          </button>
